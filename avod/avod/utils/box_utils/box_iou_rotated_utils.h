...
HOST_DEVICE_INLINE int get_intersection_points(
  ...
  {
    const Point<T>& AB = vec2[0];
    const Point<T>& DA = vec2[3];
    T ABdotAB = dot_2d<T>(AB, AB);
    T ADdotAD = dot_2d<T>(DA, DA);
    for (int i = 0; i < 4; i++) {
      // assume ABCD is the rectangle, and P is the point to be judged
      // P is inside ABCD iff. P's projection on AB lies within AB
      // and P's projection on AD lies within AD

      Point<T> AP = pts1[i] - pts2[0];

      T APdotAB = dot_2d<T>(AP, AB);
      T APdotAD = -dot_2d<T>(AP, DA);
  ...
  {
    const Point<T>& AB = vec1[0];
    const Point<T>& DA = vec1[3];
    T ABdotAB = dot_2d<T>(AB, AB);
    T ADdotAD = dot_2d<T>(DA, DA);
    for (int i = 0; i < 4; i++) {
      Point<T> AP = pts2[i] - pts1[0];
...
HOST_DEVICE_INLINE int convex_hull_graham(
  ...
  const Point<T> & start = p[t]; // starting point
  ...
  for (int i = k + 1; i < num_in; i++) {
    while (m > 1) {
      Point<T> q1 = q[i] - q[m - 2], q2 = q[m - 1] - q[m - 2];
...
single_box_iou_rotated(T const* const box1_raw, T const* const box2_raw) {
  ...
  T center_shift_x = (box1_raw[0] + box2_raw[0]) / 2.0;
  T center_shift_y = (box1_raw[1] + box2_raw[1]) / 2.0;
